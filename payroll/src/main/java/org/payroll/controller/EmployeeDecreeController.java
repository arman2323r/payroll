package org.payroll.controller;

import lombok.RequiredArgsConstructor;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.payroll.dto.SalaryDecreeDto;
import org.payroll.model.UserEntity;
import org.payroll.service.UserService;
import org.payroll.service.SalaryDecreeService;

@Controller
@RequestMapping("/employee/decree")
@RequiredArgsConstructor
public class EmployeeDecreeController {

    private final SalaryDecreeService decreeService;
    private final UserService userService;

    private UserEntity getCurrentUser() {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return userService.findByUsername(username).orElse(null);
    }

    @GetMapping("/view/{id}")
    public String view(@PathVariable Long id, Model model) {
        SalaryDecreeDto decree = decreeService.findById(id);

        UserEntity currentUser = getCurrentUser();

        if (currentUser != null && currentUser.getEmployee() != null && decree.getEmployee() != null) {
            if (!decree.getEmployee().getId().equals(currentUser.getEmployee().getId())) {
                return "redirect:/access-denied";
            }
        }

        model.addAttribute("decree", decree);
        return "employee/decree-view";
    }
}